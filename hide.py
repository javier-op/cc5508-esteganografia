...
def match_images_dimensions(image_A, image_B):
    min_y = min(len(image_A), len(image_B))
    min_x = min(len(image_A[0]), len(image_B[0]))
    image_A = image_A[:min_y, :min_x]
    image_B = image_B[:min_y, :min_x]
...
if __name__ == '__main__':
    ...
    image_A = pai_io.imread(sys.argv[1], as_gray=True)
    image_B = pai_io.imread(sys.argv[2], as_gray=True)
